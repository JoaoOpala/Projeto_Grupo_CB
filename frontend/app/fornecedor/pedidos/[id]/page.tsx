...
import { usePedidoFornecedor, useUpdateStatusPedido } from "@/lib/api/hooks/use-fornecedor";
...
export default function DetalhesPedidoFornecedorPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const { accessToken } = useAuthStore();
  const { data: pedido, isLoading, error } = usePedidoFornecedor(accessToken, id);
  const updateStatus = useUpdateStatusPedido(accessToken);
  ...
  const handleUpdateStatus = async (novoStatus: string) => {
    if (!confirm(`Alterar status para ${novoStatus.replace(/_/g, " ")}?`)) return;
    setStatusError("");
    try {
  ...
  const transitions = statusTransitions[pedido.status] || [];
        ...
        <div className="mt-6 rounded-lg border p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {transitions.map((status) => (
              ...
              >
                {updateStatus.isPending ? "Atualizando..." : status.replace(/_/g, " ")}
              </button>
            ))}
          </div>
        </div>
